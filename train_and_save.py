...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if iteration in testing_iterations:
            with torch.no_grad():
                test(scene, gaussians, deform, pipe, background, logger, logdir, iteration, devices) 
...
def train_step(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from,
    ...
    if not train_man:
        try:
            ...
            spectrum, rx_pos = next(scene.train_iter_dataset)
        except:
            scene.dataset_init()
            # spectrum, tx_pos = next(scene.train_iter_dataset)
            spectrum, rx_pos = next(scene.train_iter_dataset)
    ...
    if not train_man:
        ...
        d_signal = deform.step(gaussians.get_xyz.detach(), rx_input).squeeze(-1) 
    ...
    if not train_man:
        ...
        d_signal_bg= deform_bg.step(gaussians_bg.get_xyz.detach(), rx_input_bg, rx_orient_input_bg).squeeze(-1)
    ...
    with torch.no_grad():
        ...
        if (iteration in checkpoint_iterations):
            ...
            if not train_man:
                deform_ckpt = {
                    "deform_params": deform.deform.state_dict(),
                    "iteration": iteration
                }
                torch.save(deform_ckpt,
                        os.path.join(scene.model_path, f"deform_chkpnt{iteration}.pth"))
...
def test(scene, gaussians, deform, pipe, bg, logger, logdir, iteration, devices=torch.device('cuda'), 
    ...
    if not train_man:
        for test_input, test_label in scene.test_iter: 
            ...
            rx_orientation1 = [0, 0, 0, 1.0]
            R = torch.from_numpy(Rotation.from_quat(rx_orientation1).as_matrix()).float()
            rx_orientation1_cuda = torch.tensor(rx_orientation1).cuda()
            ...
            d_signal = deform.step(gaussians.get_xyz.detach(), rx_input_1).squeeze(-1)
            d_xyz, d_rotation, d_scaling = 0, 0, 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[400000,500000,600000, 800000,1000000,1200000])
